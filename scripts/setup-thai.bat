...
REM Thai Language Setup Script for WordPress/WooCommerce
REM ตั้งค่าภาษาไทยสำหรับ WordPress และ WooCommerce
...
REM Install Thai language packs
echo 📦 ติดตั้งแพ็คภาษาไทย...

REM Core WordPress Thai
echo 1️⃣ ติดตั้งภาษาไทยสำหรับ WordPress...
docker exec %APP_NAME%_wordpress wp language core install th --allow-root >nul 2>&1 || echo ภาษาไทยติดตั้งแล้ว

REM Plugin language packs
echo 2️⃣ ติดตั้งภาษาไทยสำหรับปลั๊กอิน...
docker exec %APP_NAME%_wordpress wp language plugin install woocommerce th --allow-root >nul 2>&1
docker exec %APP_NAME%_wordpress wp language plugin install contact-form-7 th --allow-root >nul 2>&1
docker exec %APP_NAME%_wordpress wp language plugin install wordpress-seo th --allow-root >nul 2>&1

REM Theme language packs
echo 3️⃣ ติดตั้งภาษาไทยสำหรับธีม...
docker exec %APP_NAME%_wordpress wp language theme install storefront th --allow-root >nul 2>&1
docker exec %APP_NAME%_wordpress wp language theme install twentytwentythree th --allow-root >nul 2>&1
...
REM Configure Thai-specific settings
echo 5️⃣ ตั้งค่าเฉพาะสำหรับประเทศไทย...

REM Thai Baht and formatting
...
REM Default country for WooCommerce
...
REM Thai provinces for checkout
echo 6️⃣ ตั้งค่าจังหวัดของไทย...

REM Update store address
...
REM Tax settings for Thailand (VAT 7%)
...
REM Create Thai tax rate
docker exec %APP_NAME%_wordpress wp eval "global $wpdb; $tax_rate = array('tax_rate_country' => 'TH', 'tax_rate_state' => '', 'tax_rate' => '7.0000', 'tax_rate_name' => 'VAT', 'tax_rate_priority' => 1, 'tax_rate_compound' => 0, 'tax_rate_shipping' => 1, 'tax_rate_order' => 0, 'tax_rate_class' => ''); $wpdb->insert($wpdb->prefix . 'woocommerce_tax_rates', $tax_rate);" --allow-root >nul 2>&1

REM Payment methods for Thailand
...
REM Shipping zones for Thailand
echo 9️⃣ ตั้งค่าโซนการจัดส่ง...
docker exec %APP_NAME%_wordpress wp eval "$zone = new WC_Shipping_Zone(); $zone->set_zone_name('ประเทศไทย'); $zone->add_location('TH', 'country'); $zone->save(); $zone->add_shipping_method('flat_rate'); $shipping_method = $zone->get_shipping_methods(); $method = array_shift($shipping_method); if ($method) { $method->set_option('title', 'จัดส่งทั่วประเทศ'); $method->set_option('cost', '50'); $method->save(); }" --allow-root >nul 2>&1

REM Create sample pages in Thai
echo 🔟 สร้างหน้าเพจภาษาไทย...

REM About Us page
docker exec %APP_NAME%_wordpress wp post create --post_type=page --post_title="เกี่ยวกับเรา" --post_content="<h2>ยินดีต้อนรับสู่ร้านอาหารสัตว์เลี้ยง</h2><p>เราคือผู้นำด้านอาหารและอุปกรณ์สัตว์เลี้ยงคุณภาพสูงในประเทศไทย มุ่งมั่นที่จะมอบสิ่งที่ดีที่สุดให้กับสัตว์เลี้ยงแสนรักของคุณ</p>" --post_status=publish --allow-root

REM Contact page
docker exec %APP_NAME%_wordpress wp post create --post_type=page --post_title="ติดต่อเรา" --post_content="<h2>ติดต่อเรา</h2><p><strong>ที่อยู่:</strong> 123 ถนนสุขุมวิท กรุงเทพมหานคร 10110</p><p><strong>โทร:</strong> 02-123-4567</p><p><strong>อีเมล:</strong> info@petfoodstore.co.th</p><p><strong>เวลาทำการ:</strong> จันทร์-ศุกร์ 9:00-18:00 น.</p>" --post_status=publish --allow-root

REM Shipping policy
docker exec %APP_NAME%_wordpress wp post create --post_type=page --post_title="นโยบายการจัดส่ง" --post_content="<h2>นโยบายการจัดส่งสินค้า</h2><ul><li>จัดส่งฟรีเมื่อซื้อสินค้าครบ 500 บาท</li><li>จัดส่งภายใน 1-3 วันทำการ</li><li>บริการจัดส่งทั่วประเทศไทย</li></ul>" --post_status=publish --allow-root

REM Update site title and tagline in Thai
...
REM Create menu in Thai
echo 1️⃣2️⃣ สร้างเมนูภาษาไทย...
docker exec %APP_NAME%_wordpress wp menu create "เมนูหลัก" --allow-root >nul 2>&1
